...
{
//	struct SContextFrame *ctx = (struct SContextFrame*)(next_task->stack_pointer);
	//struct SContextFrame = current_task
//	printf("next_task->stack_pointer = 0x%08x\n",next_task->stack_pointer);
	//printf("ctx->cpsr = 0x%x\n",ctx->cpsr);
//	printf("ctx->pc = 0x%x\n",ctx->pc);
...
void Do_scheduling(void)
{
	...
	Context_switch(prev_task,current_task);	
}
